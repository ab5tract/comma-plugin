...
import org.raku.pod.PodDomFormatted;
import org.raku.pod.PodDomNode;
import org.raku.pod.PodDomText;
...
public class PodFormattedImpl extends ASTWrapperPsiElement implements PodFormatted {
    ...
    public TextRange getFormattedTextRange() {
        PsiElement starter = findChildByType(org.raku.parsing.RakuTokenTypes.POD_FORMAT_STARTER);
        PsiElement stopper = findChildByType(org.raku.parsing.RakuTokenTypes.POD_FORMAT_STOPPER);
        ...
        if (content != null) {
            for (ASTNode child : content.getNode().getChildren(TokenSet.ANY)) {
                if (inLink) {
                    ...
                    if (elementType == org.raku.parsing.RakuTokenTypes.POD_FORMAT_SEPARATOR) {
