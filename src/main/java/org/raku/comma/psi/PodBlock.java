...
public interface PodBlock extends PodElement {
    TokenSet POD_CONTENT = TokenSet.create(
      ...
      RakuTokenTypes.POD_CODE,
      RakuTokenTypes.POD_NEWLINE);

    @Nullable
    default String getTypename() {
        ASTNode typenameNode = getNode().findChildByType(RakuTokenTypes.POD_TYPENAME);
    ...
    default PodDomNode buildPodDom(PodDomBuildingContext context) {
        ...
        for (ASTNode child : getContent()) {
            ...
            if (child.getElementType() == RakuTokenTypes.POD_NEWLINE) {
                ...
                if (outstandingNewlines == 1) {
                    ...
                    // newlines.
                    if (node instanceof PodDomCode && child.getElementType() == RakuTokenTypes.POD_CODE) {
            ...
            if (node == null) {
                if (child.getElementType() == RakuTokenTypes.POD_CODE) {
            ...
            else if (child.getElementType() == RakuTokenTypes.POD_CODE)
