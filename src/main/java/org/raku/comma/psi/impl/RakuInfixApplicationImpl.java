...
public class RakuInfixApplicationImpl extends RakuASTWrapperPsiElement implements RakuInfixApplication {
    ...
    private EffectCollection inferOwnEffects() {
        return isPure()
               ? EffectCollection.EMPTY
               : EffectCollection.of(Effect.IMPURE);
    }

    private boolean isPure() {
        List<RakuSymbol> symbols = resolveLexicalSymbolAllowingMulti(RakuSymbolKind.Routine, "infix:<" + getOperator() + ">");
        if (symbols.isEmpty()) return false; // If we can't resolve the operator, assume it's impure
