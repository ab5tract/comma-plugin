...
import org.raku.comma.annotation.fix.AddUnitDeclaratorQuickFix;
import org.raku.comma.annotation.fix.RemoveUnitDeclaratorQuickFix;
...
public class MissingUnitKeywordAnnotator implements Annotator {
    ...
    public void annotate(@NotNull PsiElement element, @NotNull AnnotationHolder holder) {
        if (element instanceof RakuScopedDecl) {
            ...
            if (node.getElementType() == RakuTokenTypes.STATEMENT_TERMINATOR) {
                PsiElement packageDeclarator = element.getFirstChild();
                String declaratorType = null;
                if (packageDeclarator.getNode().getElementType() == RakuTokenTypes.PACKAGE_DECLARATOR) {
